...
function EvernoteService(logger, accessToken) {
	this._logger = logger;
...
EvernoteService.prototype.search = function(filter, offset, max) { 
	...
	this._logger.log('Searching for notes with filter: ' + JSON.stringify(filter));
	...
	noteStore.findNotesMetadata(noteFilter, offset, max, resultSpec, function(error, notes) {
		if(error) {
			_self._logger.log('Got error: ' + JSON.stringify(error));
			return result.reject(error);
		}
		_self._logger.log('Got notes: ' + JSON.stringify(notes.totalNotes));
		return _self.batchGet(notes.notes)
		.then(function(items) {
			_self._logger.log('Got all notes');
			return result.resolve({
...
EvernoteService.prototype.batchGet = function(items) { 
	...
	var tasks = items.map(function(i) {
		_self._logger.log('Getting note: ' + i.guid);
...
EvernoteService.prototype.get = function(id) { 
	...
	noteStore.getNote(id, true, false, false, false, function(error, note) {
		if(error) {
			_self._logger.log('Failed to get note: ' + JSON.stringify(error));
			return result.reject(error);
		}
		_self._logger.log('Got note: ' + id);
