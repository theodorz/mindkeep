...
Service.prototype.processAll = function(batchSize) {
	...
	.then(function(result) { 
		return result.items.reduce(function(promise, item) {
...
Service.prototype.process = function(importInfo, offset) {
	...
	console.log('Starting import with filter ' + importInfo.id + ' batch offset: ' + offset + ', userId: ' + importInfo.userId);
	...
	.then(function(user) { 
		...
		return service.search(importInfo.filter, offset, 10);
	}).then(function(result) {
		...
		if(nextOffset < result.total)
			return batch.then(function() { 
				return _self.process(importInfo, offset + result.items.length);
			});
		return batch;
	}).fail(function(error) {
...
Service.prototype.processBatch = function(items, importInfo) { 
	...
	var promises = items.map(function(i) { 	
		console.log('Processing item: ' + JSON.stringify(i));
